...
import { uploadAvatar_POST, showAvatar_GET } from '../Api/Api';
...
const UserProfile = () => {
  ...
  async function avatarGet() {
    try {
      showAvatar_GET(tokenUserLocal).then((data) => setAvatar(data.avatar.url));
    } catch (error) {
      console.error('Erro ao puxar avatar do usuario', error);
    }
  }
  ...
  const uploadImage = async (e) => {
    ...
    try {
      uploadAvatar_POST(formData, tokenUserLocal);
  ...
  return (
    <div className="max-w-5xl mt-16 mx-auto cardMD:p-4 tm:p-2 animate-animeDown">
      ...
      <div className="text-slate-200 font-roboto">
        ...
        <div className="mt-12">
          <p className="text-2xl cardMD:text-lg font-mono border-b border-b-slate-800 mb-4">
            {`Minha Lista`}
          </p>
        </div>
      </div>
